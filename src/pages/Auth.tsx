...
const Auth = () => {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      {/* Left - Form */}
      <div className="flex flex-col p-8 lg:p-16 bg-background">
        <Link to="/" className="font-display text-xl font-bold text-primary mb-12">
          Salt Bee
        </Link>

        <div className="max-w-md mx-auto w-full flex-1 flex flex-col justify-center">
          <span className="inline-block self-start px-3 py-1 text-xs font-semibold rounded-full bg-primary/10 text-primary mb-4">
      ...
      <div className="hidden lg:block relative">
        <img src={authSide} alt="Korean cuisine" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-foreground/80 to-transparent" />
        <div className="absolute bottom-16 left-8 right-8 z-10">
          <div className="w-12 h-1 bg-primary mb-4" />
          <p className="text-2xl font-display italic text-primary-foreground leading-relaxed">
            "Experience the authentic flavors of Korea — where every dish tells a story of tradition and passion."
          </p>
          <p className="text-primary-foreground/70 mt-3">— The Salt Bee Team</p>
        </div>
      </div>
    </div>
  );

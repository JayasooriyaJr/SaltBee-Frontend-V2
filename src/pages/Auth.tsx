...
const Auth = () => {
  ...
  const { sessionToken, orderType } = useOrder();
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signin") {
        // Check for session conflict (Guest Session Active + Logging in)
        if (sessionToken && orderType === 'dine-in') {
  ...
  const handleLinkOrder = async () => {
    ...
    try {
      const success = await login(pendingLogin.email, pendingLogin.password, sessionToken || undefined);
      if (success) navigate('/menu');
    } catch (error) {
      toast.error("Login failed during linking.");
    } finally {
      setIsLoading(false);
  ...
  const handleKeepGuest = async () => {
    if (!pendingLogin) return;
    setIsLoading(true);
    setShowConflictModal(false);
    try {
      // Login WITHOUT session token (user session starts fresh, or whatever backend logic)
      // Ideally backend should NOT link if we don't pass the token
  ...
  // Google Login Placeholder - would ideally hook into real Google Auth provider
  const handleGoogleLogin = async () => {
    toast.info("Google Login is a placeholder in this demo.");
    // Simulated Google Token
    // await googleLogin("fake-google-token", sessionToken || undefined);
  };
      ...
      <div className="relative flex flex-col p-8 lg:p-16 bg-background overflow-hidden">
        ...
        <div className="relative z-10 max-w-md mx-auto w-full flex-1 flex flex-col justify-center">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 rounded-md border border-border bg-card text-sm font-medium text-foreground hover:bg-secondary transition-colors"
          >
